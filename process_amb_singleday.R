...
# Setup Connection to Clarity
# con <- dbConnect(odbc::odbc(), "Clarity", timeout = 10)
# access_raw <- dbGetQuery(con, "SELECT * FROM CRREPORT_REP.MV_DM_PATIENT_ACCESS WHERE ROWNUM <= 10")
# slot_raw <- dbGetQuery(con, "SELECT * FROM CRREPORT_REP.MV_DM_PATIENT_ACCESS WHERE ROWNUM <= 10")

# access_raw <- read_csv(here::here("access 2021-05-01 to 2021-06-08.csv"))
# slot_raw <- read_csv(here::here("slot 2021-09-01 to 2021-09-08.csv"))
...
# max_date_new <- max(data.subset.new$Appt.DateYear) - 1
# data.subset.new <- data.subset.new %>% filter(Appt.DateYear <= max_date_new)
...
# prev_historical <- as.data.frame(read_feather("/data/Ambulatory/Access 2020-11 to 2021-04 Slot 2020-11 to 2021-08/historical_data.feather"))
# prev_slot <- as.data.frame(read_feather("/data/Ambulatory/Access 2020-11 to 2021-04 Slot 2020-11 to 2021-08/slot_data_subset.feather"))
# prev_holid <- as.data.frame(read_feather("/data/Ambulatory/Access 2020-11 to 2021-04 Slot 2020-11 to 2021-08/holid.feather"))

# prev_historical <- as.data.frame(read_feather("C:/Users/villea04/Downloads/historical_data.feather"))
# prev_slot <- as.data.frame(read_feather("C:/Users/villea04/Downloads/slot_data_subset.feather"))
# prev_holid <- as.data.frame(read_feather("C:/Users/villea04/Downloads/holid.feather"))

# data.subset.new <- rbind(prev_historical, data.subset.new)
# slot.data.subset <- rbind(prev_slot, slot.data.subset)
# holid <- rbind(prev_holid, holid)
...
# #zipcode_ref <- read_csv(here::here("/data/Ambulatory/Data/Mappings/Oncology System Data - Zip Code Groupings 4.13.2021.csv"))
...
#zipcode <- as.data.frame(read_feather("/data/Ambulatory/Data/Mappings/zipcode.feather"))
...
# slot_prev <- as.data.frame(read_feather("Data/Updated/slot_data_subset.feather"))
#slot_dates <- sort(unique(slot.data.subset$Appt.DateYear))
#slot_prev <- slot_prev %>% filter(!(Appt.DateYear %in% slot_dates))
# slot.data.subset <- rbind(slot_prev, slot.data.subset)
# max_date_slot <- max(slot.data.subset$Appt.DateYear) - 455
# slot.data.subset <- slot.data.subset %>% filter(Appt.DateYear >= max_date_slot)
...
saveRDS(slot.data.subset, "Data/Updated/RDS/slot_data_subset.rds")
# rm(slot_raw)
# rm(slot.data.subset)


# historical_prev <- as.data.frame(read_feather("Data/Updated/historical_data (1).feather"))
# historical.data <- rbind(historical_prev, historical.data)
...
saveRDS(historical.data, "Data/Updated/RDS/historical_data.rds")
# rm(historical.data)
# rm(access_raw)



# utilization_prev <- as.data.frame(read_feather("Data/Updated/utilization_data.feather"))
# utilization.data <- rbind(utilization_prev, utilization.data)
...
saveRDS(utilization.data, "Data/Updated/RDS/utilization_data.rds")


# pop_prev <- as.data.frame(read_feather("Data/Updated/population_data_filtered.feather"))
# population.data_filtered <- rbind(pop_prev, population.data_filtered)
...
saveRDS(population.data_filtered, "Data/Updated/RDS/population_data_filtered.rds")


#saveRDS(holid, "Data/holid.rds")


# Write data frames to DATA Directory
# write_feather(historical.data, "/data/Ambulatory/Data/historical_data.feather")
# write_feather(holid, "/data/Ambulatory/Data/holid.feather")
# write_feather(population.data_filtered, "/data/Ambulatory/Data/population_data_filtered.feather")
# write_feather(slot.data.subset, "/data/Ambulatory/Data/slot_data_subset.feather")
# write_feather(utilization.data, "/data/Ambulatory/Data/utilization_data.feather")


saveRDS(holid, "Data/Updated/RDS/holid.rds")
