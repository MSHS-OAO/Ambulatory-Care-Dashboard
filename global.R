...
ambulatory_access_tbl_summary <- tbl(poolcon_production, "AMBULATORY_ACCESS_SUMMARY_TABLE")
print("summary")
ambulatory_access_tbl_summary_npr <- tbl(poolcon_production, "AMBULATORY_ACCESS_NPR_SUMMARY_TABLE")
print("npr")
...
arrivedNoShow.data.rows <- historical.data %>% filter((APPT_STATUS %in% c("No Show", "Arrived")) | (APPT_STATUS %in% c("Canceled", "Rescheduled") & LEAD_DAYS < 1 ))
...
noshow.data.rows <- historical.data %>% filter(APPT_STATUS %in% c("No Show") | (APPT_STATUS %in% c("Canceled", "Rescheduled") & LEAD_DAYS < 1 ))
