server <- function(input, output, session) {
  ...
  output$kpiApptStatusGraph <- renderPlot({
    ...
    statusDataMonth <- statusDataMonth %>%
      mutate(Cancelled = Canceled / rowSums(statusDataMonth[,3:7])) %>%
      mutate(Bumped = Bumped / rowSums(statusDataMonth[,3:7])) %>%
      mutate(NoShow = `No Show` / rowSums(statusDataMonth[,c("Arrived","No Show")])) %>%
      dplyr::select(c("Year","Month","Cancelled","Bumped","NoShow"))
    statusDataMonth[is.na(statusDataMonth)] <- 0
    ...
    statusDataDay <- statusDataDay %>%
      mutate(Cancelled = Canceled / rowSums(statusDataDay[,3:7])) %>%
      mutate(Bumped = Bumped / rowSums(statusDataDay[,3:7])) %>%
      mutate(NoShow = `No Show` / rowSums(statusDataDay[,c("Arrived","No Show")])) %>%
      dplyr::select(c("Year","Date","Cancelled","Bumped","NoShow"))
    statusDataDay[is.na(statusDataDay)] <- 0
    ...
    if(input$kpiTrend ==1){ # Historical Trend
      if(input$kpiFreq == 1){ #Year
        ...
        ggplot(dataArrived() %>% group_by(Appt.Year, Appt.Quarter) %>% 
          ...
          geom_line(color="midnightblue") +
          geom_point(color="midnightblue") +
          ggtitle("Average Check-in to Room-in Time by Quarter")+
          theme_new_line()+
          theme_bw()+
          theme(
            plot.title = element_text(hjust=0.5, face = "bold", size = 20),
        ...
        ggplot(dataArrived() %>% group_by(Appt.Year, Appt.Month) %>% 
          ...
          geom_line(color="midnightblue") +
          geom_point(color="midnightblue") +
          ggtitle("Average Check-in to Room-in Time by Month")+
          theme_new_line()+
          theme_bw()+
          theme(
            plot.title = element_text(hjust=0.5, face = "bold", size = 20),
        ...
        ggplot(dataArrived() %>% group_by(Appt.Year, Appt.Date) %>% 
                 summarise(mean = mean(checkinToRoomin, na.rm=TRUE)), aes(x=interaction(Appt.Year,as.Date(Appt.Date, format="%Y-%m-%d"),lex.order = TRUE), y=mean,group=1)) +
          geom_line(color="midnightblue") +
          ggtitle("Average Check-in to Room-in Time by Day")+
          theme_new_line()+
          theme_bw()+
          theme(
            plot.title = element_text(hjust=0.5, face = "bold", size = 20),
  ...
  output$canceledLeadDays <- renderPlot({
    ...
    ggplot(cancellations, aes(x=factor(leadDays, levels = status), y=percent))+
      geom_bar(stat="identity", fill="midnightblue", width=0.8)+
  ...
  output$ins_breakdown <- renderPlot({
  ...
  output$population1 <- renderLeaflet({
    ...
    newdata <- population.data %>% group_by(latitude, longitude) %>% summarise(total = round(n(),0))
