...
if(out_of_date == 'TRUE'){
  ...
  missing_dates <- unique(date(data.subset.new.missing$APPT.DTTM))
...
if (utilization_flag == 1){
  missing_arrivedNoshow.data <- filter(arrivedNoshow.data, APPT.DTTM %in% missing_dates)
  utilization_output <- pre_process_utilization(max_date,missing_arrivedNoshow.data)
  new_data.hour.arrived <- utilization_output[[2]]
  new_data.hour.scheduled <- utilization_output[[1]]
  data.hour.arrived <- rbind(data.hour.arrived,new_data.hour.arrived)
  data.hour.scheduled <- rbind(data.hour.scheduled,new_data.hour.scheduled)
  data.hour.arrived <- data.hour.arrived %>% filter(Appt.DTTM >= max_date - 60*24*60*60)
  data.hour.scheduled <- data.hour.scheduled %>% filter(Appt.DTTM >= max_date - 60*24*60*60)
  
} else{
  utilization_output <- pre_process_utilization(max_date,arrivedNoshow.data)
  data.hour.arrived <- utilization_output[[2]] 
  data.hour.scheduled <- utilization_output[[1]] 
