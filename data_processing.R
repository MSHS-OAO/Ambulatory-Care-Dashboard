...
max_date_data_type <- function(data_type){
  if(!is.null(data_type$SLOT_BEGIN_TIME)){
    max_date_data_all <- date(max(slot.data.subset$SLOT_BEGIN_TIME))
  }else{
    max_date_data_all <- date(max(data.subset.new$Appt.DTTM))
  }
...
max_date_slot <- max_date_data_type(slot.data.subset)
